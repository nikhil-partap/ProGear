...
export default function ProductCard({ product }) {
  return <article className={'collection-card ' + (product.id === 'universal' ? 'universal-card' : '')}>
    <Link className="collection-image" to={'/product/' + product.id} aria-label={'Shop ' + product.name}>
      {product.images.length ? <img src={product.id === 'premium' ? product.images[1] : product.images[0]} alt={product.name + ' ' + product.type + ' car mat'} loading="lazy" /> : <div className="universal-art"><strong>Universal</strong><p>Trim-to-fit mats</p><span>Photo coming soon</span></div>}
