...
export default function Home() {
  ...
  return <main id="main-content">
    <section className={'hero hero-slide-' + slide} aria-label="ProGear collections">
      <div className="hero-photos">{heroSlides.map((item, index) => item.mobileImage ? <picture key={`${item.id}-${index}`}><source media="(max-width: 900px)" srcSet={item.mobileImage} /><img src={item.image} className={index === slide ? 'active' : ''} alt={item.name + ' car mats, ' + item.type} fetchPriority={index === 0 ? 'high' : 'auto'} /></picture> : <img key={`${item.id}-${index}`} src={item.image} className={index === slide ? 'active' : ''} alt={item.name + ' car mats, ' + item.type} fetchPriority={index === 0 ? 'high' : 'auto'} />)}</div>
